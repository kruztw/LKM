...
static void hello_iowrite(void *opaque, hwaddr addr, uint64_t value, unsigned size)
{
    ...
    // just for test interrupt handlers
    // cat /proc/interrupts , you will see IRQ <id> has been triggered once
...
static void pci_hellodev_init(PCIDevice *pci_dev, Error **errp)
{
    ...
    sleep(1);
    //object_property_set_bool(OBJECT(d), true, "realized", errp);
}
